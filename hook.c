...
static int replace_call_func(unsigned long handler, unsigned long orig_func,
    ...
    if (*tmp_addr == 0xe8) {
      ...
      if (((unsigned long)tmp_addr + 5 + *offset) == orig_func) {
        ...
        *offset = custom_func - (unsigned long)tmp_addr - 5;
        pr_info("base addr: 0x%08lx, offset:%08lx, replace func: %08lx to "
                "func: %08lx.\n",
...
inline int install_hook(syscall_fn_t *sys_call_table,
...
inline int uninstall_hook(syscall_fn_t *sys_call_table,
                          struct syscall_hook *hook) {
  int res = 0;
